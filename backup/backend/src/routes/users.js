...
router.get('/profile', authenticateToken, async (req, res) => {
  try {
    ...
    EnterpriseLogger.error('Get profile error', error, { userId: req.user.id, ip: req.ip });
...
router.put('/profile', authenticateToken, async (req, res) => {
  try {
    ...
    EnterpriseLogger.info('Profile update', req.user.id, {
    ...
    EnterpriseLogger.error('Update profile error', error, { userId: req.user.id, updates: Object.keys(req.body), ip: req.ip });
...
router.get('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    EnterpriseLogger.error('Get user error', error, { userId: req.params.id, requesterId: req.user.id, ip: req.ip });
...
router.post('/video', authenticateToken, videoUpload.single('video'), async (req, res) => {
  try {
    ...
    EnterpriseLogger.error('Video upload error', error, { userId: req.user.id, ip: req.ip });
...
router.get('/video/:userId', authenticateToken, async (req, res) => {
  try {
    ...
    EnterpriseLogger.error('Get video error', error, { userId: req.user.id, ip: req.ip });
...
router.delete('/video', authenticateToken, async (req, res) => {
  try {
    ...
    EnterpriseLogger.info('Deleting video', req.user.id, {
      action: 'delete_user_video'
    });

    res.json({ message: 'Video deleted successfully' });
  } catch (error) {
    EnterpriseLogger.error('Delete video error', error, { userId: req.user.id, ip: req.ip });
...
router.post('/video/verify', authenticateToken, async (req, res) => {
  try {
    ...
    EnterpriseLogger.error('Video verification error', error, { userId: req.user.id, ip: req.ip });
...
router.get('/video/verification/:verificationId', authenticateToken, async (req, res) => {
  try {
    ...
    EnterpriseLogger.error('Get verification status error', error, { userId: req.user.id, ip: req.ip });
